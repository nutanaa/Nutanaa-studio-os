...
export class AuditManager extends Disposable implements IAuditManager {
	...
	public readonly onDidSecurityAlert = this._onDidSecurityAlert.event;
	public readonly onDidCriticalEvent = this._onDidCriticalEvent.event;
	...
	archiveEvents(beforeDate: number): number {
		...
		this.events.splice(0, toArchive.length);
