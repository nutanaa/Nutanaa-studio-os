...
import { IAuthenticationManager } from '../common/authenticationManager.js';
...
export class AuthenticationManager extends Disposable implements IAuthenticationManager {
	...
	public readonly onDidLogin = Event.fromEmitter(this._onDidLogin);
	public readonly onDidLogout = Event.fromEmitter(this._onDidLogout);
	public readonly onDidSessionExpire = Event.fromEmitter(this._onDidSessionExpire);
	...
	private loadUsers(): void {
		...
		if (stored) {
			try {
				...
				this.userPasswords = new Map(data.passwords || []);
			} catch {
				this.users = new Map();
	...
	private saveUsers(): void {
		...
		this.storageService.store('nutanaa.users', JSON.stringify(data), 0);
	}
}