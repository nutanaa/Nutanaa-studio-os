...
import { IBackupManager } from '../common/backupManager.js';
...
export class BackupManager extends Disposable implements IBackupManager {
	...
	private readonly _onDidFailBackup = this._register(new Emitter<{ error: string }>());

	public readonly onDidCompleteBackup = Event.fromEmitter(this._onDidCompleteBackup);
	public readonly onDidCompleteRestore = Event.fromEmitter(this._onDidCompleteRestore);
	public readonly onDidFailBackup = Event.fromEmitter(this._onDidFailBackup);
	...
	updateConfig(config: Partial<IBackupConfig>): void {
		...
		this.storageService.store('backup.config', JSON.stringify(this.config), 0);
	...
	startScheduledBackups(): void {
		...
		this.scheduledInterval = setInterval(() => {
			this.createBackup('incremental').catch(error => {
				this._onDidFailBackup.fire({ error: String(error) });
			});
	...
	private saveBackups(): void {
		...
		this.storageService.store('backup.backups', JSON.stringify(data), 0);
		...
		this.runtimeStateService.update({
			production: {
				...
				backupStatus: latestBackup ? {
					...
					nextBackup: this.getNextScheduledBackup() || 0,
