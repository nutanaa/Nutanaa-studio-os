...
import { ILoggingManager } from '../common/loggingManager.js';
...
export class LoggingManager extends Disposable implements ILoggingManager {
	...
	public readonly onDidAddLog = Event.fromEmitter(this._onDidAddLog);
	...
	setLevel(level: LogLevel): void {
		this.storageService.store('logging.level', level, 0);
	...
	setCategoryLevel(category: string, level: LogLevel): void {
		...
		this.storageService.store('logging.categoryLevels', JSON.stringify(levels), 0);
	}
	...
	private saveLogs(): void {
		...
		this.storageService.store('nutanaa.logs', JSON.stringify(recentLogs), 0);
	}
}