...
import { IMarketplaceService } from '../common/marketplaceService.js';
...
export class MarketplaceService extends Disposable implements IMarketplaceService {
	...
	public readonly onDidUpdatePlugin = Event.fromEmitter(this._onDidUpdatePlugin);

	private readonly _onDidRemovePlugin = this._register(new Emitter<{ pluginId: string }>());
	public readonly onDidRemovePlugin = Event.fromEmitter(this._onDidRemovePlugin);
	...
	private saveCache(): void {
		...
		this.storageService.store('nutanaa.marketplace.cache', JSON.stringify(data), 0);
