...
export class MemoryManager extends Disposable implements IMemoryManager {
	...
	public readonly onDidChangeMemory = this._onDidChangeMemory.event;
	...
	createMemory(entry: Omit<IMemoryEntry, 'id' | 'timestamp' | 'lastAccessedTimestamp' | 'accessCount'>): string {
		...
		this.memoryOrder = [...this.memoryOrder, id];
	...
	deleteMemory(memoryId: string): boolean {
		...
		if (orderIndex !== -1) {
			this.memoryOrder = [
				...this.memoryOrder.slice(0, orderIndex),
				...this.memoryOrder.slice(orderIndex + 1)
			];
		}
	...
	async searchMemories(query: string, options: IMemorySearchOptions): Promise<IMemorySearchResult[]> {
		...
		for (const [_id, memory] of Object.entries(this.memories)) {
			// Filter by type
	...
	compressMemories(maxAgeMs: number, keepCount: number): number {
		...
		if (excessCount > 0) {
			...
			this.memoryOrder = this.memoryOrder.slice(excessCount);
