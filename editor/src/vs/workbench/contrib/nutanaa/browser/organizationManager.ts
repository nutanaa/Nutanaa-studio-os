...
import { IOrganizationManager } from '../common/organizationManager.js';
...
export class OrganizationManager extends Disposable implements IOrganizationManager {
	...
	public readonly onDidOrganizationChange = Event.fromEmitter(this._onDidOrganizationChange);
	public readonly onDidTeamChange = Event.fromEmitter(this._onDidTeamChange);
	...
	getSharedResources(organizationId: string): Map<string, Array<{ id: string; name: string; type: string }>> {
		...
		for (const [resourceType, resources] of orgResources) {
			...
			for (const [resourceId, permissions] of resources) {
	...
	private loadData(): void {
		const stored = this.storageService.get('nutanaa.organizations', 0);
		if (stored) {
			try {
				...
				for (const [orgId, memberships] of Object.entries(data.memberships || {})) {
					this.teamMemberships.set(orgId, memberships);
				}
				for (const [orgId, resources] of Object.entries(data.sharedResources || {})) {
					const orgResources = new Map();
					for (const [type, typeResources] of Object.entries(resources)) {
						const typedResources = new Map(Object.entries(typeResources as Record<string, unknown>));
