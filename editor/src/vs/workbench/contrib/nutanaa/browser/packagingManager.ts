...
import { IPackagingManager, IPackageBuildInfo, IInstallerInfo, IArtifactInfo } from '../common/packagingManager.js';
...
class PackagingManager extends Disposable implements IPackagingManager {
	...
	async buildPackage(platform: 'windows' | 'linux' | 'darwin' | 'portable'): Promise<{
		...
		const buildPromise = new Promise<{ success: boolean; artifactPath?: string; error?: string }>((resolve) => {
	...
	async generateInstaller(type: 'msi' | 'deb' | 'rpm' | 'dmg' | 'pkg' | 'exe', platform: string): Promise<string> {
		const installerId = `${type}-${platform}-${Date.now()}`;
