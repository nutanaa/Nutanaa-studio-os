...
import { IProviderManager, IProviderManager as IProviderManagerConst } from '../common/providerManager.js';
...
import { RequestMethod, RESTApiProvider } from './providers/restApiProvider.js';
...
export class ProviderManager extends Disposable implements IProviderManager {
	...
	public readonly onDidChangeHealth = Event.fromEmitter(this._onDidChangeHealth);
	public readonly onDidChangeSelection = Event.fromEmitter(this._onDidChangeSelection);
	public readonly onDidChangeProviders = Event.fromEmitter(this._onDidChangeProviders);
	...
	selectProvider(request: IProviderRequest): IProviderSelectionResult | undefined {
		...
		return {
			provider: selected,
			loadBalancingDecision: `Selected ${selected.name} (priority: ${selected.priority}, load: ${load}/${this.getProviderMaxConcurrency(selected.name)})`,
		};
	}
	...
	getProviderLoad(providerName: string): number | undefined {
		...
		const max = this.getProviderMaxConcurrency(providerName);
