...
import { IRecoveryManager } from '../common/recoveryManager.js';
...
export class RecoveryManager extends Disposable implements IRecoveryManager {
	...
	public readonly onDidCrash = Event.fromEmitter(this._onDidCrash);
	public readonly onDidRecover = Event.fromEmitter(this._onDidRecover);
	...
	updateConfig(config: Partial<IRecoveryConfig>): void {
		...
		this.storageService.store('recovery.config', JSON.stringify(this.config), 0);
	...
	private storageStore(key: string, value: string): void {
		this.storageService.store(key, value, 0);
	}

	private storageDelete(key: string): void {
		// StorageService doesn't have delete, but we can overwrite with empty
		this.storageService.store(key, '', 0);
	}
}