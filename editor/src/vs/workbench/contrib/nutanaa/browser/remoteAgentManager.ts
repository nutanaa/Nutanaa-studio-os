...
import { IRemoteAgentManager } from '../common/remoteAgentManager.js';
...
export class RemoteAgentManager extends Disposable implements IRemoteAgentManager {
	...
	public readonly onDidAgentConnect = Event.fromEmitter(this._onDidAgentConnect);
	public readonly onDidAgentDisconnect = Event.fromEmitter(this._onDidAgentDisconnect);
	public readonly onDidAgentStatusChange = Event.fromEmitter(this._onDidAgentStatusChange);
	public readonly onDidMissHeartbeat = Event.fromEmitter(this._onDidMissHeartbeat);
	...
	async failoverToBackup(agentId: string): Promise<IRemoteAgent> {
		...
		try {
			...
			try {
				const state = await this.syncAgentState(agentId);
		...
		if (stored) {
			try {
				...
				for (const [primaryId, backups] of Object.entries(data.backups || {})) {
					this.backupAgents.set(primaryId, backups);
				}
			} catch {
				this.agents = new Map();
