...
export class DistributedRuntimeManager extends Disposable implements IDistributedRuntimeManager {
		...
		if (stored) {
			try {
				...
				for (const [id, node] of Object.entries(data.nodes || {})) {
					this.nodes.set(id, node as INodeInfo);
				}
	...
	onDidNodeJoin(listener: (node: INodeInfo) => void) {
		return this._onDidNodeJoin.event(listener);
	}

	onDidNodeLeave(listener: (nodeId: string, reason: string) => void) {
		return this._onDidNodeLeave.event(e => listener(e.nodeId, e.reason));
	}

	onDidNodeStatusChange(listener: (nodeId: string, status: INodeInfo['status']) => void) {
		return this._onDidNodeStatusChange.event(e => listener(e.nodeId, e.status));
	}

	onDidMasterChange(listener: (oldMasterId: string | undefined, newMasterId: string) => void) {
		return this._onDidMasterChange.event(e => listener(e.oldMasterId, e.newMasterId));
	}
}