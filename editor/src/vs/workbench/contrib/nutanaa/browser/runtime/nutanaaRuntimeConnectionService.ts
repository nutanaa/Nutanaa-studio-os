...
export class NutanaaRuntimeConnectionService extends Disposable implements INutanaaRuntimeConnectionService {
	...
	private readonly backendEventMap: Record<string, RuntimeEventType> = {
		'task.execution.started': RuntimeEventType.AgentStarted,
		'task.execution.completed': RuntimeEventType.AgentCompleted,
		'task.execution.failed': RuntimeEventType.AgentFailed,
		'task.execution.cancelled': RuntimeEventType.AgentCancelled,
		'task.execution.stream_chunk': RuntimeEventType.AgentStream,
	};
	...
	async executeAgent(name: string, payload: Record<string, unknown>): Promise<{
		success: boolean;
		output?: unknown;
		error?: string;
	}> {
		if (this._state !== NutanaaRuntimeConnectionState.Connected) {
			throw new Error('Not connected to runtime backend');
		}
		try {
			const response = await fetch(`${NUTANAA_RUNTIME_HTTP_URL}/agents/${encodeURIComponent(name)}/execute`, {
				method: 'POST',
				headers: { 'Content-Type': 'application/json' },
				body: JSON.stringify(payload),
			});
			if (!response.ok) {
				throw new Error(`Backend /agents/${name}/execute returned HTTP ${response.status}`);
			}
	...
	private handleMessage(data: unknown): void {
		...
		let message: { type?: string; source?: string; subjectId?: string; payload?: unknown };
		try {
			message = JSON.parse(data);
		} catch {
			return;
		}
		const mappedType = message.type ? this.backendEventMap[message.type] : undefined;
		if (mappedType) {
			this.eventBus.fire({
				type: mappedType,
				timestamp: Date.now(),
		...
		} else if (message.type && message.type !== 'heartbeat') {
