...
export class PluginManager extends Disposable implements IPluginManager {
	...
	async loadPlugin(pluginId: string): Promise<IPlugin> {
		...
		try {
			...
			const loadingPlugin = { ...plugin, state: 'loading' as const };
			this.plugins.set(pluginId, loadingPlugin);

			// Check dependencies first
			const dependencies = await this.checkDependencies(loadingPlugin);
			...
			// Mark as loaded
			const loadedPlugin = { ...plugin, state: 'loaded' as const, error: undefined };
			...
			const updatedPlugin = { ...plugin, state: 'error' as const, error: String(error) };
	...
	async unloadPlugin(pluginId: string): Promise<void> {
		...
		try {
			const unloadingPlugin = { ...plugin, state: 'unloading' as const };
			this.plugins.set(pluginId, unloadingPlugin);
			...
			const installedPlugin = { ...plugin, state: 'installed' as const };
	...
	enablePlugin(pluginId: string): void {
		...
		if (updatedPlugin.state === 'installed') {
		...
		if (stored) {
			try {
				...
				for (const [id, plugin] of Object.entries(data.plugins || {})) {
					this.plugins.set(id, plugin as IPlugin);
				}
			} catch {
				this.plugins.clear();
			}
	...
	private savePlugins(): void {
		...
		this.storageService.store('nutanaa.plugins', JSON.stringify({ plugins: pluginsObj }), StorageScope.APPLICATION, StorageTarget.USER);
	}

	onDidInstallPlugin(listener: (plugin: IPlugin) => void) {
		return this._onDidInstallPlugin.event(listener);
	}

	onDidUninstallPlugin(listener: (pluginId: string) => void) {
		return this._onDidUninstallPlugin.event(e => listener(e.pluginId));
	}

	onDidLoadPlugin(listener: (plugin: IPlugin) => void) {
		return this._onDidLoadPlugin.event(listener);
	}

	onDidUnloadPlugin(listener: (pluginId: string) => void) {
		return this._onDidUnloadPlugin.event(e => listener(e.pluginId));
	}

	onDidPluginError(listener: (pluginId: string, error: string) => void) {
		return this._onDidPluginError.event(e => listener(e.pluginId, e.error));
	}
}