...
import { ITracingManager } from '../common/tracingManager.js';
...
export class TracingManager extends Disposable implements ITracingManager {
	...
	public readonly onDidCreateTrace = Event.fromEmitter(this._onDidCreateTrace);
	public readonly onDidCompleteSpan = Event.fromEmitter(this._onDidCreateTrace);

	private readonly MAX_TRACES = 1000;
	private readonly MAX_SPANS_PER_TRACE = 10000;
	...
	endTrace(traceId: string, status: 'ok' | 'error' | 'cancelled'): void {
		...
		this.runtimeEventBus.fire({
			type: RuntimeEventType.TraceCreated,
			timestamp: Date.now(),
			payload: {
				traceId,
				type: trace.type,
