...
export interface IBackupManager {
	 ...
	 */
	onDidCompleteBackup: (listener: (backup: IBackup) => void) => { dispose(): void };

	/**
	 * Event fired when restore completes.
	 */
	onDidCompleteRestore: (listener: (backupId: string, success: boolean) => void) => { dispose(): void };

	/**
	 * Event fired when backup fails.
	 */
	onDidFailBackup: (listener: (error: string) => void) => { dispose(): void };
}