 ...
 *
 * The runtime backend (`runtime/main.py`) is a separate FastAPI/WebSocket
 * process the editor connects to over HTTP + WebSocket (Phase 1). It only
 * exposes a health check and a heartbeat socket so far — no agents,
 * workflows, or providers are wired through it yet (Phase 2+). Until then,
 * {@link INutanaaRuntimeConnectionService} implementations must honestly
 * report an empty agent list rather than synthesizing fake data, even
 * while {@link NutanaaRuntimeConnectionState.Connected}.
 */
