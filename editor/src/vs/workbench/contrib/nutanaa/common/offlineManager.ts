...
export interface IOfflineManager {
	 ...
	 */
	onDidChangeStatus: (listener: (isOnline: boolean) => void) => { dispose(): void };
	 ...
	 */
	onDidSync: (listener: (success: boolean, synced: number, failed: number) => void) => { dispose(): void };
	 ...
	 */
	onDidDetectConflict: (listener: (conflict: IConflictResolution) => void) => { dispose(): void };
