...
export interface IOrganizationManager {
	 ...
	 */
	onDidOrganizationChange: (listener: (organizationId: string, changeType: 'joined' | 'left' | 'updated') => void) => { dispose(): void };

	/**
	 * Event fired when team changes.
	 */
	onDidTeamChange: (listener: (teamId: string) => void) => { dispose(): void };
}