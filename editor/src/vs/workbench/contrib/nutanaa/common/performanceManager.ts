...
export interface IPerformanceManager {
	 ...
	 */
	onDidDetectSlowTask: (listener: (taskId: string, taskName: string, duration: number) => void) => { dispose(): void };

	/**
	 * Event fired when frame drop is detected.
	 */
	onDidDetectFrameDrop: (listener: (frameTime: number, threshold: number) => void) => { dispose(): void };
}