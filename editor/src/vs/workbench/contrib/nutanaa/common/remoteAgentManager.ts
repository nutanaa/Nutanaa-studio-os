...
import { IRemoteAgent, INodeInfo } from '../models/enterpriseModel.js';
...
export interface IRemoteAgentManager {
	 ...
	 */
	onDidAgentConnect: (listener: (agent: IRemoteAgent) => void) => { dispose(): void };

	/**
	 * Event fired when agent disconnects.
	 */
	onDidAgentDisconnect: (listener: (agentId: string, reason: string) => void) => { dispose(): void };

	/**
	 * Event fired when agent status changes.
	 */
	onDidAgentStatusChange: (listener: (agentId: string, status: IRemoteAgent['status']) => void) => { dispose(): void };

	/**
	 * Event fired when heartbeat is missed.
	 */
	onDidMissHeartbeat: (listener: (agentId: string) => void) => { dispose(): void };
}